...
class Astar():
    def __init__(self, map_orig):
        ...
        self.visited_nodes = [36893488147419103233] # 2**64 + 1 防空报错
        self.priority_heap = []
        ...
        for i in range(1, 31): # [0, 9] + [0, 2] * 10 + 1 -> [1, 30]
            ...
            for link in itertools.combinations(pts, 2):
                ...
                if cost is not None:
                    ...
                    if unvisited_pos !=  self.visited_nodes[bisect.bisect_left(self.visited_nodes, unvisited_pos)]:
                        n = Node(node, link, unvisited_pos, node.cost + cost - PREDC)
