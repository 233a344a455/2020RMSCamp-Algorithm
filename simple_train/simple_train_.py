...
class simple_train_one_num:
    ...
    def __init__(self, data, labels, toler, step_w, step_b):
        ...
        self.w = np.zeros(
            (int(len(self.num_kind) * (len(self.num_kind) - 1) / 2), self.train_data_dim))
        self.b = np.zeros(
            (int(len(self.num_kind) * (len(self.num_kind) - 1) / 2), 1))
    # 计算标签集中的数字种类
    ...
    def train_learn_two_num(self, i, j):
        ...
        # train_data (60000, 28*28) -> shape(w) = (784,)
        b = 0
        ...
        while train_flag:
            ...
            for data, label in zip(self.train_data, self.train_label):
                ...
                # 判断是第一个数字还是第二个数字
                # 大于0为第一数， 小于为第二数
                if label == self.num_kind[i]:
                    y = 1
                elif label == self.num_kind[j]:
                    y = -1
